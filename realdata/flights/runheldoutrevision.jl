...
if type == 1
    include("/home/jlederman/DiscreteOrderStatistics/models/flights/flights_final.jl")
    model = flights(Ntrain, M, T, R, Dmax, a, b, c, d, alpha, beta)
    if D > 0
        t = @elapsed samples = fit(model, data, nsamples = Nsamples, nburnin=nbunrin, nthin=nthin, mask=nothing, info=info, initseed=chainSeed,
        verbose =true)
...
elseif type == 2
    ...
    model = flights_STAR(Ntrain, M, R, alpha, beta, tau2, g1, g_inv1)
    t = @elapsed samples = fit(model, data, nsamples = Nsamples, nburnin=nbunrin, nthin=nthin, mask=nothing, info=info, initseed=chainSeed,
    verbose =true)
    samplesnew = [Dict("U_R"=> s["U_R"], "sigma2_R" => s["Dsigma2_RR"]) for s in samples]
end
