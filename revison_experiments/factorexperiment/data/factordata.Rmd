...
# Plot heatmap
library(pheatmap)
pdf("structured_poisson_heatmap.pdf", width = 8, height = 6)
...
pheatmap(
  ...
  legend = TRUE,
  fontsize = 40,        # affects legend text
  main = 
)
...
write.csv(Theta, "CMPU_NK2.csv")
write.csv(Phi, "CMPV_KM.csv")   # revision 2: save true Phi as well
```



```{r}
x <- rcmp(10000,  Lambda[1,1], nu = 2)
 var(x)/mean(x)
```
...
pheatmap(Theta, cluster_rows = FALSE, cluster_cols = FALSE,
         color = colorRampPalette(c("white","red"))(100),
         main = paste0("Structured Poisson Factor Data Heatmap, K=", K))
write.csv(Theta, "PoisU_NK.csv")  # revision 2: save true factors
write.csv(Phi, "PoisV_KM.csv")
```
...
# Generate Poisson observations
Y <- matrix(rnbinom(J*I, size=as.vector(Lambda), prob= .25), nrow = J, ncol = I)
Y <- t(Y)  # rows = observations
...
write.csv(Theta, "NBU_NK.csv")  # revision 2: save true factors
write.csv(Phi, "NBV_KM.csv")
```
...
write.csv(Theta, "GCU_NK.csv")  # revision 2: save true factors
write.csv(Phi, "GCV_KM.csv")
```

```{r}
x <- rgc(4, Lambda[1,1],100000)
var(x)/mean(x)
```
```{r}
