...
struct covid3PoissonB <: MatrixMF
    N::Int64
    M::Int64
    K::Int64
    a::Float64
...
function evalulateLogLikelihood(model::covid3PoissonB, state, data, info, row, col)
...
function sample_prior(model::covid3PoissonB, info=nothing,constantinit=nothing)
    pass = false
...
function forward_sample(model::covid3PoissonB; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::covid3PoissonB, data, state, mask=nothing;skipupdate=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    ...
    @views @threads for idx in 1:(model.N * model.M)
        ...
        probvec = U_NK[n,:] .* V_KM[:,m] #.* lambda_K
        mu1 = sum(probvec) 
        mu = Ylast + eps + mu1
        if !isnothing(mask)
            if mask[n,m] == 1
