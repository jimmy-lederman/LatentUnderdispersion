...
struct covid3PoissonA <: MatrixMF
    N::Int64
    M::Int64
    K::Int64
    a::Float64
...
function evalulateLogLikelihood(model::covid3PoissonA, state, data, info, row, col)
    ...
    rate = Ylast+alpha*pop*mu
    return logpdf(Poisson(rate), Y)
end

function sample_prior(model::covid3PoissonA, info=nothing,constantinit=nothing)
    pass = false
    ...
    state = Dict("U_NK" => U_NK, "V_KM" => V_KM, "alpha"=>alpha,
    "Y0_N"=>info["Y0_N"], "pop_N"=>info["pop_N"], "pop_sum"=>pop_sum)
    return state
end

function forward_sample(model::covid3PoissonA; state=nothing, info=nothing)
    if isnothing(state)
    ...
    for m in 1:model.M
        for n in 1:model.N
            if m == 1
                Y_NM[n,m] = rand(Poisson(Y0_N[n] + pop_N[n]*alpha*sum(U_NK[n,:] .* V_KM[:,m])))
            else
                Y_NM[n,m] = rand(Poisson(Y_NM[n,m-1] + pop_N[n]*alpha*sum(U_NK[n,:] .* V_KM[:,m])))
            end
        end 
    end
    data = Dict("Y_NM" => Y_NM)
...
function backward_sample(model::covid3PoissonA, data, state, mask=nothing;skipupdate=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    ...
    @views @threads for idx in 1:(model.N * model.M)
        ...
        if Y_NM[n, m] > 0
            ...
            @inbounds begin
                sum_factor_thr[tid] += Y_NM[n,m] - y_k[model.K+1]
            end
    ...
    C1_K = vec(alpha * sum(pop_N .* U_NK, dims=1))
