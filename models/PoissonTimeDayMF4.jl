...
function evalulateLogLikelihood(model::PoissonTimeDayMF, state, data, info, row, col)
    ...
    return logpdf(Poisson(delta*Ylast + alpha*pop*mu + pop*eps), Y)
end
...
function predict(model::PoissonTimeDayMF, state, info, n, m, Ylast)
    ...
    s = state_N[n]
    pop = pop_N[n]
    return rand(Poisson(delta*Ylast + pop*eps + alpha * pop * sum(state["R_KTS"][:,day,s] .* state["U_NK"][n,:] .* state["V_KM"][:,m])))
end
        ...
        @views for m in 1:model.M
            @views for k in 1:(model.K+2) 
                ...
                elseif m == 1 && k == (model.K+1)
                    ...
                    if k == (model.K+1)
                        Y_NMKplus1[n,m,k] = rand(Poisson(delta*sum(Y_NMKplus1[n,m-1,:])))
                    elseif k ==  (model.K+2)
...
function backward_sample(model::PoissonTimeDayMF, data, state, mask=nothing,skipupdate=nothing)
    ...
    @views @threads for idx in 1:(N * (M))
        ...
        if !isnothing(mask)
            if mask[n,m] == 1
                Y_NM[n,m] = rand(Poisson(delta*Ylast + pop*eps + sum(alpha*probvec)))
            end
        end
        if Y_NM[n, m] > 0
    ...
    post_rate = model.Ylastrate + sum(Y_NM[:,1:(end-1)])
