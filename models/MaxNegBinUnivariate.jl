...
struct MaxNegBinUnivariate <: MatrixMF
    N::Int64
    M::Int64
    a::Float64
    b::Float64
...
function sample_prior(model::MaxNegBinUnivariate, info=nothing)
...
function forward_sample(model::MaxNegBinUnivariate; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::MaxNegBinUnivariate, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    mu = copy(state["mu"])
    p = copy(state["p"])
    ...
    for n in 1:model.N
        if Y_NM[n,1] > 0
            Z1_N[n] = sampleSumGivenMax(Y_NM[n,1], model.D, NegativeBinomial(mu, 1-p))
