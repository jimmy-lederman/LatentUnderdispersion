...
function evalulateLogLikelihood(model::covid2, state, data, info, row, col)
...
function sample_prior(model::covid2, info=nothing,constantinit=nothing)
    pass = false
...
function forward_sample(model::covid2; state=nothing, info=nothing)
    if isnothing(state)
    ...
    for m in 1:model.M
        for n in 1:model.N
            if m == 1
                ...
                Y_NM[n,m] = rand(Poisson(Y_NM[n,m-1] + eps + sum(U_NK[n,:] .* V_KM[:,m])))
            end
        end 
    end
    data = Dict("Y_NM" => Y_NM)
...
function backward_sample(model::covid2, data, state, mask=nothing;skipupdate=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
...
function predict(model::covid2, state, info, n, m, Ylast)
    eps = state["eps"]
    U_NK = state["U_NK"]
    V_KM = state["V_KM"]

    return rand(Poisson(Ylast + eps  + sum(U_NK[n,:] .* V_KM[:,m])))
end

function predict_x(model::covid2, state, info, n, mstart, Ystart, x)
    result = zeros(x)
    Ylast = Ystart
    m = mstart + 1
...
function forecast(model::covid2, state, data, info, Ti)
