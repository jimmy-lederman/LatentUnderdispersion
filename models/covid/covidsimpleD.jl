...
function backward_sample(model::covidsimple, data, state, mask=nothing; skipupdate=nothing)
    ...
    @views for m in 1:model.M
        @views for k in 1:model.K
            if m == 1
                V_KM[k,m] = rand(Gamma(model.starta + Y_MK[m,k] + l_KM[k,m+1], 1/(model.startb + C1_KM[k,m] + model.c*q_KM[k,m+1])))
    ...
    if isnothing(skipupdate) || !("D_NM" in skipupdate)
        ...
        @views @threads for m in 1:model.M
            W = Diagonal(W_NM[:,m]) 
            V = inv(Beta_NQ' * W * Beta_NQ + (1/sigma_M[m])*Matrix{Int}(I, model.Q, model.Q))
            V = .5(V + V')
