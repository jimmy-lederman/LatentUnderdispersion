...
function evalulateLogLikelihood(model::flightsbase, state, data, info, row, col)
    ...
    mu = A_T[home] + B_T[away] + dist*state["U_K"][state["Z_TT"][home,away]]
...
function sample_likelihood(model::flightsbase, mu,D,n=1)
    if D == 1
        if n == 1
...
function sample_prior(model::flightsbase, info=nothing)
    ...
    state = Dict("U_K" => U_K, "A_T" => A_T, "B_T" => B_T, "Z_TT"=>Z_TT,
...
function forward_sample(model::flightsbase; state=nothing, info=nothing)
    ...
    for n in 1:model.N
        ...
        away = away_N[n]
        route = routes_N[n]
        mu = A_T[home] + B_T[away] + dist_N[n,1]*U_K[Z_TT[home,away]]
...
function backward_sample(model::flightsbase, data, state, mask=nothing)
    ...
    #@views @threads for r in 1:R
        ...
        Z_TTnew[home,away] = argmax(rand(Gumbel(0,1), model.K) + logprobvec_K)
    ...
    state = Dict("U_K" => U_K, "A_T"=>A_T, "B_T"=>B_T, "Z_TT"=>Z_TTnew,
