...
struct MaxPoissonMFhorse <: MatrixMF
    N::Int64
    M::Int64
    K::Int64
    a::Float64
...
function evalulateLogLikelihood(model::MaxPoissonMFhorse, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
    mu = dot(state["U_NK"][row,:], state["V_KM"][:,col])
    return logpmfMaxPoisson(Y,mu,model.D)
end

function sample_prior(model::MaxPoissonMFhorse,info=nothing)
...
function forward_sample(model::MaxPoissonMFhorse; state=nothing, info=nothing)
    if isnothing(state)
        state = sample_prior(model)
    end
    regiment_M = info["regiment_M"]
    # println(size(regiment_M))
    # println(size(state["V_KM"]))
    Mu_NM = state["U_NK"] * (regiment_M'.* state["V_KM"])
    ...
    # end
    data = Dict("Y_NM" => Y_NM)
    state = Dict("U_NK" => state["U_NK"], "V_KM" => state["V_KM"], "regiment_M"=>info["regiment_M"])
    return data, state 
end

function backward_sample(model::MaxPoissonMFhorse, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    U_NK = copy(state["U_NK"])
    V_KM = copy(state["V_KM"])
    regiment_M = copy(info["regiment_M"])
    Z_NM = zeros(Int, model.N, model.M)
    Z_NMK = zeros(Int, model.N, model.M, model.K)
    #Z_NMK = copy(state["Z_NMK"])
    Mu_NM = U_NK * (regiment_M'.* V_KM)
    ...
    @views for n in 1:model.N
        @views for k in 1:model.K
            ...
            post_rate = model.b + model.D*sum(V_KM[k, :] .* regiment_M)
    ...
    @views for m in 1:model.M
        @views for k in 1:model.K
            ...
            post_rate = model.d + regiment_M[m]*model.D*sum(U_NK[:, k])
            V_KM[k, m] = rand(Gamma(post_shape, 1/post_rate))[1]
        end
    end
    state = Dict("U_NK" => U_NK, "V_KM" => V_KM, "regiment_M"=>info["regiment_M"])
    return data, state
end
