...
function sample_prior(model::flights_STAR, info=nothing, constantint=nothing)
    ...
    sigma2_R = rand(InverseGamma(model.c, model.d), model.R)
    #sigma2 = rand(InverseGamma(model.c, model.d))

    state = Dict("U_R" => U_R, "A_T" => A_T, "B_T" => B_T, "beta"=>beta, "sigma2_R"=>sigma2_R,
...
function backward_sample(model::flights_STAR, data, state, mask=nothing; skipupdate=nothing)
    ...
    @views for r in 1:model.R
        ...
        V = 1 / (1 + (distance^2 * n) / sigma2)
        m = V * (distance * s / sigma2)
    ...
    @views for t in 1:model.T
        ...
        for i in 1:length(indices)
            f = indices[i]
            sigma2 =sigma2_R[routes[i]]
            @assert distances[i] == 1
            s += (Z_NM[f,1] - B_T[destinations[i]] - U_R[routes[i]] * distances[i])/sigma2
            s2 += 1 / sigma2
        end
    ...
    @views for t in 1:model.T
        ...
        for i in 1:length(indices)
            ...
            sigma2 = sigma2_R[routes[i]]
            @assert distances[i] == 1
            s += (Z_NM[f,1] - A_T[origins[i]] - U_R[routes[i]] * distances[i])/sigma2
            s2 += 1 / sigma2
        end
    ...
    @views for r in 1:model.R
        ...
        distance = routes_R4[r,4]
        @assert distance == 1
        s = sum((Z_NM[indices,1] .- (A_T[home] + B_T[away] + U_R[r]*distance)).^2)
        n = length(indices)
        sigma2_R[r] = rand(InverseGamma(model.c + n/2, model.d+ s/2))
    end

    # s = sum((Z_NM .- A_T[home_N] .- B_T[away_N] .- U_R[routes_N] .* dist_N).^2)
    ...
    state = Dict("U_R" => U_R, "A_T"=>A_T, "B_T"=>B_T,"sigma2_R"=>sigma2_R,"beta"=>beta,
