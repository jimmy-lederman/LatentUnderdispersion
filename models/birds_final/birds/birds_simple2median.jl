...
function evalulateLogLikelihood(model::birdsmedian, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
...
# function sample_D(model::birdsmedian, Y, mu, p)
...
function sample_prior(model::birdsmedian,info=nothing, constantinit=nothing)
    ...
    D_NM = 2*rand.(Binomial.(Int((model.Dmax - 1)/2), logistic.(Tau_NP * Beta_PM))) .+ 1
...
function forward_sample(model::birdsmedian; state=nothing, info=nothing, constantinit=nothing)
    if isnothing(state)
    ...
    Y_NM = rand.(OrderStatistic.(Poisson.(Mu_NM), D_NM, div.(D_NM, 2) .+ 1))
    data = Dict("Y_NM" => Y_NM)

    return data, state 
end

function backward_sample(model::birdsmedian, data, state, mask=nothing;skipupdate=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    ...
    #for idx in 1:(model.N * model.M)
        ...
        if !isnothing(mask)
            if mask[n,m] == 1
                Y_NM[n,m] = rand(OrderStatistic(Poisson(Mu_NM[n,m]),  D_NM[n,m],  div(D_NM[n,m], 2) + 1))
            end
        end
        
        
        Z_NM[n, m] = sampleSumGivenOrderStatistic(Y_NM[n, m], D_NM[n,m], div(D_NM[n,m], 2) + 1, Poisson(Mu_NM[n, m]))
        if Z_NM[n,m] > 0
            P_K = U_NK[n, :] .* V_KM[:, m]
