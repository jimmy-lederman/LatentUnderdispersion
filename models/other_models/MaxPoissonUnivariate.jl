...
struct MaxPoissonUnivariate <: MatrixMF
    N::Int64
    M::Int64
    a::Float64
    b::Float64
...
function evalulateLogLikelihood(model::MaxPoissonUnivariate, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
    mu = state["mu"]
    return logpmfMaxPoisson(Y,mu,model.D)
end

function sample_prior(model::MaxPoissonUnivariate, info=nothing)
    mu = rand(Gamma(model.a, 1/model.b))
    state = Dict("mu" => mu)
    return state
end

function forward_sample(model::MaxPoissonUnivariate; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::MaxPoissonUnivariate, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    mu = copy(state["mu"])
    Z_N = zeros(model.N)
    ...
    for n in 1:model.N
        if Y_NM[n,1] > 0
            Z_N[n] = sampleSumGivenMax(Y_NM[n,1], model.D, Poisson(mu))
        end
    end
