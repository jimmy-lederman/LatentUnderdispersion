...
struct MaxPoissonCovariateMF <: MatrixMF
    N::Int64
    M::Int64
    K::Int64
    a::Float64
...
function evalulateLogLikelihood(model::MaxPoissonCovariateMF, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
...
function sample_prior(model::MaxPoissonCovariateMF,info=nothing)
...
function forward_sample(model::MaxPoissonCovariateMF; state=nothing, info=nothing)
    if isnothing(state)
    ...
    Y_NM = rand.(OrderStatistic.(Poisson.(Mu_NM1 + Mu_NM2.*dist_NM), model.D, model.D))
...
function backward_sample(model::MaxPoissonCovariateMF, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    ...
    for n in 1:model.N
        for m in 1:model.M
            if !isnothing(mask)
                if mask[n,m] == 1
                    Y_NM[n,m] = rand(OrderStatistic(Poisson(Mu_NM1[n,m] + dist_NM[n,m]*Mu_NM2[n,m]), model.D, model.D))
                    if Y_NM[n,m] > 20 println(Y_NM[n,m]) end
                end
            end
            if Y_NM[n, m] > 0
                Z_NM[n, m] = sampleSumGivenMax(Y_NM[n, m], model.D, Poisson(Mu_NM1[n,m] + dist_NM[n,m]*Mu_NM2[n,m]))
    ...
    for n in 1:model.N
        for k in 1:model.K
            ...
            post_rate = model.b + model.D*sum(V_KM1[k, :])
    ...
    for m in 1:model.M
        for k in 1:model.K
            ...
            post_rate = model.d + model.D*sum(U_NK1[:, k])
    ...
    for n in 1:model.N
        for k in 1:model.K
            ...
            post_rate = model.f + model.D*sum(V_KM2[k, :].*dist_NM[n,:])
