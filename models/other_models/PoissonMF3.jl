...
function evalulateLogLikelihood(model::PoissonMF3, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
    mu = dot(state["U_NK"][row,:], state["V_KM"][:,col])
    return logpdf(Poisson(mu), Y)
end

function sample_prior(model::PoissonMF3,info=nothing,constantinit=nothing)
    ...
    state = Dict("U_NK" => U_NK, "V_KM" => V_KM, "a_K"=>a_K, "phi_KM"=>phi_KM)
    return state
end

function forward_sample(model::PoissonMF3; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::PoissonMF3, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
