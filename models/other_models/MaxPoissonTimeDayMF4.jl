...
function forecast(model::MaxPoissonTimeDayMF, state, data, info, Ti)
    ...
    for i in 1:Ti
        mu = state["delta"]*Ylast .+ info["pop_N"].*(state["eps"] .+ state["alpha"]*mu_NTi[:,i])
    ...
    for n in 1:model.N
        for m in 1:model.M
            if m > 1
                day = days_M[m]
                s = state_N[n]
                mu = delta*Y_NM[n,m-1] + pop_N[n]*eps + alpha * pop_N[n] * sum(state["R_KTS"][:,day,s] .* state["U_NK"][n,:] .* state["V_KM"][:,m])
                Y_NM[n,m] = rand(OrderStatistic(Poisson(mu), model.D, model.D))
            end
        end
    end
...
function predict(model::MaxPoissonTimeDayMF, state, info, n, m, Ylast)
    ...
    pop = pop_N[n]
    delta = state["delta"]
    return rand(OrderStatistic(Poisson(delta*Ylast + pop*eps + alpha * pop * sum(state["R_KTS"][:,day,s] .* state["U_NK"][n,:] .* state["V_KM"][:,m])), model.D, model.D))
end
...
function backward_sample(model::MaxPoissonTimeDayMF, data, state, mask=nothing, skipupdate=nothing)
    ...
    @views @threads for idx in 1:(N * (M))
        ...
        if !isnothing(mask)
            if mask[n,m] == 1   
                Y_NM[n,m] = rand(OrderStatistic(Poisson(delta*Ylast + pop*eps + alpha*mu), model.D, model.D))
        ...
        if Y_NM[n, m] > 0
            ...
            Z = sampleSumGivenMax(Y_NM[n, m], model.D, Poisson(delta*Ylast + alpha*mu + pop*eps))

            probvec = vcat(alpha*probvec, delta*Ylast, pop*eps)
    ...
    post_shape = model.Ylastshape + sum(Y_NMKplus1[:,:,model.K+1])
