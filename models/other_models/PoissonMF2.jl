...
function evalulateLogLikelihood(model::PoissonMF2, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
    mu = dot(state["U_NK"][row,:], state["V_KM"][:,col])
    return logpdf(Poisson(mu), Y)
end

function sample_prior(model::PoissonMF2,info=nothing,constantinit=nothing)
...
function forward_sample(model::PoissonMF2; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::PoissonMF2, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
