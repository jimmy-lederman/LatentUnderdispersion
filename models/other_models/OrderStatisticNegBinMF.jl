...
function griddy_gibbs(model::OrderStatisticNegBinMF, U_NK, Z_MK, Z_NM, plist=.01:.005:.99)#plist=[.49,.5,.51,.52,.53,.55,.6]))
    ...
    for (i,p) in enumerate(plist)
        ...
        # accumulate the grid log-likelihood without materializing mu_NM, the array
        # of NegativeBinomial objects, or the array of logpdfs (4 N*M temporaries per
        # grid point x length(plist) points). Same quantity; only the summation order
        # differs from `sum` of a broadcast array.
        acc = 0.0
        @inbounds for m in 1:model.M
            for n in 1:model.N
                ...
                acc += logpdf(NegativeBinomial(model.D*mu, p), Z_NM[n,m])
            end
        end
   ...
   # if !isnothing(mask)
    ...
    if griddy
        ...
        @views @threads for idx in 1:(model.N * model.M)
            ...
            # Sample the CRT count for each of the D latent siblings, then thin the
            # TOTAL once. The per-d thinnings all used the same weights P_K, and only
            # their sum is ever accumulated into Z_NK/Z_MK; multinomials with a common
            # weight vector are additive, so one draw on sum(z_d) has exactly the same
            # distribution as summing D separate draws.
            ztot = 0
            @inbounds for d in 1:model.D
                ztot += sampleCRT(Z_D[d], Mu_NM[n,m])
            end
            if ztot > 0
                z_k = thin_multinomial!(zk_thr[tid], ztot, P_K, model.K)
                @inbounds for k in 1:model.K
                    Z_NK_thr[tid][n, k] += z_k[k]
                    Z_MK_thr[tid][m, k] += z_k[k]
                end
