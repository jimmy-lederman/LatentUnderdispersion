...
struct MaxPoissonDiagonal <: MatrixMF
    N::Int64
    M::Int64
    T::Int64
    K::Int64
...
function evalulateLogLikelihood(model::MaxPoissonDiagonal, state, data, info, row, col)
    Y = data["Y_NM"][row,col]
...
function sample_prior(model::MaxPoissonDiagonal, info=nothing)
...
function forward_sample(model::MaxPoissonDiagonal; state=nothing, info=nothing)
    if isnothing(state)
...
function backward_sample(model::MaxPoissonDiagonal, data, state, mask=nothing)
    #some housekeeping
    Y_NM = copy(data["Y_NM"])
    ...
    @views @threads for n in 1:model.N
        ...
        if Y_NM[n, 1] > 0
            Z = sampleSumGivenMax(Y_NM[n, 1], model.D, Poisson(MU_TT[home, away]))
            P_K = U1_TK[home, :] .* U2_TK[away, :] .* V_K
            P_K[:] = P_K / sum(P_K)
            Y_NMK[n, 1, :] = rand(Multinomial(Z, P_K))
        end
        if Y_NM[n, 2] > 0
            Z = sampleSumGivenMax(Y_NM[n, 2], model.D, Poisson(MU_TT[away, home]))
            P_K = U1_TK[away, :] .* U2_TK[home, :] .* V_K
            P_K[:] = P_K / sum(P_K)
            Y_NMK[n, 2, :] = rand(Multinomial(Z, P_K))
        end
    end
    ...
    @views for k in 1:model.K
        ...
        post_rate = model.d + model.D*PHI3_K[k]
