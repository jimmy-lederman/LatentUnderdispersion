...
DATA = Dict(
  ...
  "SparsePois"=> (joinpath(HERE, "data/SparsePoisfactor.csv"), joinpath(HERE, "data/SparsePoisU_NK.csv")),
...
function build(mname, ac, N, M, K)
    ...
    # sqrt variants: on heavy-tailed data g(x)=x badly handicaps STAR -- with
    # sqrt its info rate on SparsePois is -0.972 against -1.727 for identity,
    # better than both Poisson (-1.113) and MedPois (-1.042). STAR must be judged
    # on its best transformation or the comparison is a strawman.
    elseif mname == "starnn_slice_sqrt"; return STARMFNNsp(N, M, K, ac, ac, 0.01, 1.0, 1.0, sqrt, x -> x^2, :slice)
    elseif mname == "starnn_mh_sqrt";    return STARMFNNsp(N, M, K, ac, ac, 0.01, 1.0, 1.0, sqrt, x -> x^2, :mh)
    elseif mname == "star_g";   return STARMF(N, M, K, 1.0, 1.0, identity, identity)   # a,c do not apply
    else error("unknown model $mname") end
end

function run_one(ds, mname, ac, seed; NS=2000, NB=2000, NC=4)
    Y = readm(DATA[ds][1]); N, M = size(Y); K = 3
    raw = readm(DATA[ds][2])
    Utrue = size(raw, 1) == N ? Float64.(raw) : Float64.(permutedims(raw))
    data = Dict("Y_NM" => Y)
    model = build(mname, ac, N, M, K)
    Random.seed!(seed)
    mask = rand(N, M) .< 0.2
