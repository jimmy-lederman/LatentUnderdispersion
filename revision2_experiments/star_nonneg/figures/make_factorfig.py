...
Keynote version.
  ...
  panel (a)  the observed count matrix
  panel (b)  true factor loadings beside each model's posterior mean loadings,
             including the STAR-NN variant added in revision 2
...
COLOR. Non-negative models use a sequential white-to-sienna scale. Gaussian
STAR uses a diverging blue-white-red scale centered at zero because its loadings
genuinely go negative; that is the visual point of the panel, and a sequential
scale would conceal it. Each tile is scaled to its own maximum, since factors
are identified only up to a scale exchanged with phi -- only the PATTERN is
comparable across models, never the magnitude.
...
# (model key, label, Gaussian prior?); "truth" is the reference tile.
# Laid out as two rows of four, the last cell left blank.
PANELS = [("truth", "Truth", False),
          ("medpois", "MedPoisson", False),
          ("mednb", "MedNB", False),
...
SEQ = LinearSegmentedColormap.from_list("wsienna", ["white", "#A1522D"])
...
# The left ratio is set so the square 20x20 panel roughly fills its box: at this
# figure size the plot area is ~3.9in tall, so a wider box would just pad panel
# (a) with whitespace rather than enlarging the heatmap.
outer = fig.add_gridspec(1, 2, width_ratios=[0.55, 1.25], wspace=0.06,
                         left=0.03, right=0.985, top=0.90, bottom=0.06)

axa = fig.add_subplot(outer[0, 0])
im = axa.imshow(Y, cmap=LinearSegmentedColormap.from_list("wred", ["white", "#C62828"]),
...
for s in axa.spines.values():
    s.set_linewidth(0.6)
fig.colorbar(im, ax=axa, fraction=0.030, pad=0.02)

grid = outer[0, 1].subgridspec(2, 4, wspace=0.18, hspace=0.32)
...
# Panel labels and the divider are positioned in FIGURE coordinates, so the
# figure is saved without bbox_inches="tight" -- tight cropping rescales those
# coordinates after the fact and silently moves both.
DIV = 0.35
